...
export async function updateCustomerTaskStatus({ projectId, taskId, email, customer, action, note, request }) {
  ...
  const title =
    action === "approve"
      ? `Task approved by client: ${task.taskCode || task.title}`
      : `Revision requested by client: ${task.taskCode || task.title}`;
  const message =
    action === "approve"
      ? `${customer?.name || "The client"} approved "${task.title}".`
      : `${customer?.name || "The client"} requested revisions on "${task.title}".`;
  if (task.assigneeId) {
    await createNotification({
      role: "DEVELOPER",
      userId: task.assigneeId,
      type: action === "approve" ? "TASK_STATUS" : "APPROVAL_REQUEST",
      title,
      message,
      entityType: "Task",
      entityId: task.id,
  ...
  if (project.projectManagerId && project.projectManagerId !== task.assigneeId) {
    await createNotification({
      role: "DEVELOPER",
